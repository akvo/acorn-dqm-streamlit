...
st.caption(
    "Analyzes data quality metrics by individual enumerator. Use this to identify enumerators who may need additional training, equipment checks, or supervision. Compare error rates, review specific error types, and export individual performance reports for team management."
)
...
def capture_map_as_image(enum_data, enumerator_name):
    ...
    try:
        ...
        try:
            ...
            # Set title
            ax.set_title(
                f"Geographic Distribution - {enumerator_name}",
                fontsize=18,
                fontweight="bold",
                color="#1565C0",
                pad=20,
            ...
            legend_elements = [
                Patch(
                    facecolor="#4CAF50",
                    edgecolor="#2E7D32",
                    label=f"Valid Subplots ({valid_count})",
                ),
                Patch(
                    facecolor="#F44336",
                    edgecolor="#C62828",
                    label=f"Invalid Subplots ({invalid_count})",
                ),
            ]
            ax.legend(
...
def create_subplot_polygon_image(subplot_row):
    ...
    try:
        ...
        print(
            f"DEBUG POLYGON: Starting for subplot {subplot_id}",
            file=sys.stderr,
        )
        print(
            f"DEBUG POLYGON: Has geometry column: {'geometry' in subplot_row}",
            file=sys.stderr,
        )
        ...
        if pd.isna(subplot_row.get("geometry")):
            print(
                f"DEBUG POLYGON: Geometry is NaN for subplot {subplot_id}",
                file=sys.stderr,
            )
            return None
        ...
        if geom.is_empty:
            print(
                f"DEBUG POLYGON: Geometry is empty for subplot {subplot_id}",
                file=sys.stderr,
            )
            return None
...
def generate_enhanced_pdf_report(
    ...
    try:
        ...
        if (
            ...
            print(
                f"DEBUG PDF: plots_df columns: {plots_df.columns.tolist()}",
                file=sys.stderr,
            )
            print(
                f"DEBUG PDF: enum_data columns: {enum_data.columns.tolist()}",
                file=sys.stderr,
            )
            ...
            if submission_date_col and subplot_key_col:
                ...
                print(
                    "DEBUG PDF: Could not find required columns in plots_df",
                    file=sys.stderr,
                )
        else:
            if "SubmissionDate" in enum_data.columns:
                print(
                    "DEBUG PDF: SubmissionDate already in enum_data",
                    file=sys.stderr,
                )
            else:
                print(
                    "DEBUG PDF: No raw_data or plots_subplots available to add SubmissionDate",
                    file=sys.stderr,
                )
                        ...
                        if len(meas_enum) > 0:
                            ...
                            if "plots_subplots_vegetation" in raw_data:
                                ...
                                if (
                                    ...
                                    if (
                                        ...
                                        print(
                                            "DEBUG: Merged subplot_comments for PDF",
                                            file=sys.stderr,
                                        )
                            ...
                            if (
                                ...
                                if len(height_check) > 0:
                                    ...
                                    print(
                                        f"DEBUG: Found {len(outliers)} height outliers",
                                        file=sys.stderr,
                                    )
                                    print(
                                        f"DEBUG: Height outliers columns: {outliers.columns.tolist()}",
                                        file=sys.stderr,
                                    )
                                    print(
                                        f"DEBUG: meas_enum columns: {meas_enum.columns.tolist()}",
                                        file=sys.stderr,
                                    )
                                    print(
                                    ...
                                    if (
                                        ...
                                        if (
                                            ...
                                            print(
                                                "DEBUG: Merged SubmissionDate using MEASUREMENT_KEY",
                                                file=sys.stderr,
                                            )
                                        elif (
                                            "VEGETATION_KEY"
                                            ...
                                            print(
                                                "DEBUG: Merged SubmissionDate using VEGETATION_KEY",
                                                file=sys.stderr,
                                            )
                            ...
                            print(
                                f"DEBUG: Total vegetation errors: {total_veg_errors}",
                                file=sys.stderr,
                            )
                except Exception as e:
                    # If vegetation error detection fails, continue without it
                    print(
                        f"DEBUG: Error in vegetation error detection: {str(e)}",
                        file=sys.stderr,
                    )
                    import traceback
        ...
        print(
            f"DEBUG PDF: Total missing veg for enumerator: {total_missing_veg}",
            file=sys.stderr,
        )
        ...
        if "SubmissionDate" in enum_data.columns:
            ...
            if len(dates) > 0:
                ...
                for date_idx, date in enumerate(dates):
                    ...
                    date_summary_data = [
                        ...
                        [
                            "Valid",
                            f"{date_valid} ({date_valid / date_total * 100:.1f}%)",
                        ],
                        [
                            "Invalid",
                            f"{date_invalid} ({date_error_rate:.1f}%)",
                        ],
                    ]
                    ...
                    if len(date_data) > 0 and MATPLOTLIB_AVAILABLE:
                        print(
                            f"DEBUG PDF: date_data columns: {date_data.columns.tolist()}",
                            file=sys.stderr,
                        )
                        print(
                            f"DEBUG PDF: PLOT_KEY in columns: {'PLOT_KEY' in date_data.columns}",
                            file=sys.stderr,
                        )
                        if "PLOT_KEY" in date_data.columns:
                            ...
                            print(
                                f"DEBUG PDF: GT Plot keys: {date_data['PLOT_KEY'].unique()}",
                                file=sys.stderr,
                            )
                        ...
                        if "PLOT_KEY" in date_data.columns:
                                ...
                                try:
                                    ...
                                    legend_elements = [
                                        Patch(
                                            facecolor="#4CAF50",
                                            alpha=0.5,
                                            label=f"Valid ({plot_valid})",
                                        ),
                                        Patch(
                                            facecolor="#F44336",
                                            alpha=0.6,
                                            label=f"Invalid ({plot_invalid})",
                                        ),
                                    ]
                                    ax.legend(
                                        handles=legend_elements,
                            ...
                            try:
                                ...
                                story.append(Spacer(1, 0.2 * inch))
                                print(
                                    "DEBUG PDF: Successfully added map overview to PDF",
                                    file=sys.stderr,
                                )

                            except Exception as e:
                                print(
                                    f"DEBUG PDF: Error creating map overview: {str(e)}",
                                    file=sys.stderr,
                                )
                                import traceback
                    ...
                    if len(date_invalid_data) > 0 or len(date_missing_veg) > 0:
                        ...
                        for idx, row in date_invalid_data.iterrows():
                            ...
                            if MATPLOTLIB_AVAILABLE:
                                try:
                                    if (
                                        ...
                                        polygon_img_rl = RLImage(
                                            poly_buffer,
                                            width=2.5 * inch,
                                            height=2.08 * inch,
                                        )
                                        print(
                                            "DEBUG PDF: Successfully created polygon image inline",
                                            file=sys.stderr,
                                        )
                                    else:
                                        print(
                                            "DEBUG PDF: Geometry is None or empty",
                                            file=sys.stderr,
                                        )

                                except Exception as e:
                                    print(
                                        f"DEBUG PDF: Error creating polygon inline: {str(e)}",
                                        file=sys.stderr,
                                    )
                                    import traceback
                                ...
                                print(
                                    "DEBUG PDF: Matplotlib not available, skipping polygon creation",
                                    file=sys.stderr,
                                )
                            ...
                            if polygon_img_rl:
                                ...
                                print(
                                    "DEBUG PDF: Added 2-column table with polygon",
                                    file=sys.stderr,
                                )
                            else:
                                detail_table = Table(
                        ...
                        story.append(
                            Paragraph(
                                "✓ All subplots geometrically valid for this date",
                                success_note_style,
                            )
                        )
                    ...
                    if len(date_missing_veg) > 0:
                        ...
                        story.append(
                            Paragraph(
                                "❌ Missing Vegetation Records",
                                missing_veg_header_style,
                            )
                        )
                    ...
                    if veg_errors_data:
                        ...
                        print(
                            f"DEBUG: Total date_veg_errors collected: {len(date_veg_errors)}",
                            file=sys.stderr,
                        )
                        if date_veg_errors:
                            story.append(
                                Paragraph(
                                    "🌿 Vegetation & Measurement Issues",
                                    veg_error_header_style,
                                )
                            )
                            ...
                            veg_error_table = Table(
                                veg_error_table_data,
                                colWidths=[1.5 * inch, 2.5 * inch, 2.3 * inch],
                            )
            ...
            if len(invalid_data) > 0:
                ...
                subplot_table = Table(
                    subplot_details,
                    colWidths=[1.8 * inch, 2.2 * inch, 2 * inch],
                )
        ...
        print(
            f"DEBUG PDF: Building final PDF with {len(story)} elements",
            file=sys.stderr,
        )
        doc.build(story)
        buffer.seek(0)
        print(
            "DEBUG PDF: PDF generation complete, returning buffer",
            file=sys.stderr,
        )
        return buffer
...
with tabs[TAB_OVERVIEW]:
    ...
    st.caption(
        "Compares validation error rates across selected enumerators. Higher error rates (red) may indicate training needs, equipment issues, or challenging field conditions. The stacked chart shows absolute counts of valid vs invalid subplots per enumerator."
    )
...
with tabs[TAB_GEOMETRY]:
    ...
    st.caption(
        "Breaks down GPS/geometry validation failures by error type per enumerator. Common issues include: area too small/large, insufficient GPS accuracy, overlapping subplots. Patterns help identify whether issues are enumerator-specific (training) or equipment-specific (device problems)."
    )
...
with tabs[TAB_ERROR_DETAILS]:
    ...
    st.caption(
        "Deep dive into a single enumerator's data quality. Shows: summary metrics, interactive map of their subplots, detailed geometry errors, and vegetation/measurement outliers. Use this for one-on-one feedback sessions or to prepare individual training plans."
    )
    ...
    if selected_enum:
        ...
        if (
            ...
            enum_data = enum_data.drop(
                columns=["subplot_number", "measured_subplots_int"],
                errors="ignore",
            )
        ...
        st.caption(
            "All subplots collected by this enumerator. Green = valid, Red = invalid. Click subplots for detailed validation info. Use this to spot geographic patterns in errors (e.g., issues concentrated in specific areas may indicate terrain challenges)."
        )
        ...
        st.caption(
            "List of this enumerator's subplots that failed geometry validation. Review 'reasons' column for specific issues. Area/vertices/ratios help diagnose whether the problem is GPS accuracy, plot shape, or boundary demarcation."
        )
        ...
        st.caption(
            "Measurement quality issues for this enumerator: missing vegetation records, height/circumference outliers, and suspicious age-circumference combinations. These may indicate measurement technique issues that need coaching."
        )
        ...
        st.caption(
            "Export this enumerator's data for offline review, performance discussions, or record-keeping. CSV for spreadsheet analysis, PDF for printable reports, GeoJSON for GIS software, Errors Only for focused remediation lists."
        )
